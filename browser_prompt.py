...
async def answer_generator_browser(
    user_question,
    ...
    system_prompt = f"""
        ...
        • `examples` (if present)
        • `special_context_chunks` (If `IMAGE_DATA` is True, you MUST use this source. It is the absolute highest priority.)
        ...
        2. Locate the minimum set of statements from the sources that  
            2.1  **MANDATORY FORCE RULE**: When `IMAGE_DATA` is True and `special_context_chunks` are provided (not empty), you **MUST** include at least one solution or insight derived from them.
                 - **Priority #1**: The content from `special_context_chunks` MUST be presented FIRST in your answer.
                 - **No Filtering**: Do NOT filter this content out even if you think it is low relevance, low probability, or partial.
                 - **No Judgement**: Even if it seems like a "weak match" to the question, you MUST present it as a possible solution path.
                 - Only after presenting the `special_context_chunks` solution can you add details from `database_occurances` or `knowledge_base`.
                 - Do not mention or label sources (no “from special chunks”, no “from DB”, etc.).
            2.2  Otherwise, primary sources are `database_occurances` and `knowledge_base`.  
        ...
        4. Compose a fact-based, concise answer that directly addresses the user’s question. Ensure:
            ...
            • If the sources include explicit steps/procedure to solve the task, you must include those steps in the answer (do not omit them).
        5. If the sources conflict, acknowledge the discrepancy briefly (in one sentence), then present the more reliable/explicit statement and quote it; do not invent a reconciliation and do not identify or label sources. 
        ...
        *Example of correct usage:*
        ...
        9. If `database_occurances` and `knowledge_base` are empty, and `special_context_chunks` are empty (or `IMAGE_DATA` is False), respond with a single polite sentence explaining that you couldn’t find relevant information. Do not speculate.
    ...
    if is_brief_mode:
        system_prompt += (
            ...
            "2. Use plain sentences (no headings, no lists, no tables) unless the user explicitly requests a list. If the sources include explicit steps/procedure, include them as concise step-by-step sentences (inline numbering is allowed).\n"
    ...
    if is_voice_mode:
        system_prompt += (
            ...
            "\n"
            "### Opening with conversational transitions (mandatory)\n"
            "Before writing, read the last assistant message in `conversation_history`.\n"
            "- REQUIRED: Every answer MUST open with a natural spoken transition word. "
            "This is non-negotiable in voice mode — answers that jump straight into facts sound robotic when read aloud.\n"
            f"- LANGUAGE RULE: The opener MUST be in `{language}` — the same language as the entire answer. "
            "Never use English openers when responding in another language, and never mix languages at the start.\n"
            f"- Pick a natural spoken transition in `{language}`. "
            "English examples (use ONLY when responding in English): 'Also,', 'Hmm,', 'Well,', 'So,'. "
            "For any other language, use the closest natural equivalent in that language — not a literal English copy.\n"
            "- ROTATION RULE: If the immediately preceding assistant message started with a specific transition word, "
            "you MUST pick a DIFFERENT one. Never use the exact same opener twice in a row.\n"
            "- Rotate across turns so each answer feels fresh and human.\n"
            "\n"
            ...
            f"- Always lead with a conversational transition in `{language}` before the core content. "
            "This is what makes the answer sound spoken, not written.\n"
            "- Alternate which transition you use — never repeat the exact same one from the previous turn.\n"
            "- Vary rhythm within the answer: mix short and longer sentences.\n"
            f"- Use natural spoken transitions mid-sentence when helpful — always in `{language}`.\n"
            ...
            f"using phrases in `{language}`. Never invent facts.\n"
            "\n"
            ...
            '(e.g. "Want me to walk through the next step?"). '
            ...
            "\n"
            "### Opening and length patterns (no fixed scripts)\n"
            "Rotate which transition you use — never reuse the same template across turns. "
            f"All openers must be in `{language}`:\n"
            "- Continuation — when adding to or continuing a topic.\n"
            "- Pause — when thinking or introducing something nuanced.\n"
            "- Reframe — when giving context before the answer.\n"
            "- Direct — when getting straight to the point.\n"
            "BAD: Starting without any transition word — sounds like a document, not a voice message.\n"
            "BAD: Using an English opener when the answer is not in English.\n"
            "BAD: Using the same opener as the previous assistant message.\n"
            "BAD: 5+ sentences or multiple paragraphs covering every detail from the sources.\n"
            "\n"
            ...
            f"- Your answer STARTS with a natural spoken transition in `{language}`. "
            "If it does not, rewrite the opening. "
            "If the first word is in a different language than the rest of the answer, rewrite it.\n"
            "- Compare the first word with the last assistant message in `conversation_history`. "
            "If you reused the exact same transition word, rewrite to use a different one.\n"
    ...
    user_prompt = build_user_prompt(
        ...
        special_chunks_when_image_only=True,
    )
