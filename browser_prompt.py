...
async def answer_generator_browser(
    ...
    if is_voice_mode:
        system_prompt += (
            ...
            "- Pick your opener from this set: 'Also,', 'Hmm,', 'Well,', 'So,' "
            "(or the closest natural equivalent in the response language).\n"
            "- ROTATION RULE: If the immediately preceding assistant message started with a specific transition word "
            "(e.g., 'Also,'), you MUST pick a DIFFERENT one from the set above. "
            "Never use the exact same opener twice in a row.\n"
            ...
            "- Always lead with a conversational transition ('Also,', 'Hmm,', 'Well,', 'So,') before the core content. "
            ...
            "Rotate which transition word you use — never reuse the same template across turns:\n"
            "- 'Also,' — when adding to or continuing a topic.\n"
            "- 'Hmm,' — when pausing to think or introducing something nuanced.\n"
            "- 'Well,' — when reframing or giving context before the answer.\n"
            "- 'So,' — when getting straight to the point after setup.\n"
            ...
            "- Your answer STARTS with one of: 'Also,', 'Hmm,', 'Well,', 'So,'. "
            "If it does not, rewrite the opening to include one.\n"
            "- Compare the first word with the last assistant message in `conversation_history`. "
            "If you reused the exact same transition word, rewrite to use a different one from the set.\n"
