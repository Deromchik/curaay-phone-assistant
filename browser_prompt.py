...
async def answer_generator_browser(
    ...
    if is_voice_mode:
        system_prompt += (
            ...
            "Before writing, read the last assistant messages in `conversation_history`.\n"
            "- Do NOT start with the same opening word, filler, or phrase used in any of them "
            "(in any language — e.g. avoid repeating \"Well,\" \"So,\" \"Yeah,\" \"Right,\" "
            "or equivalent discourse markers).\n"
            "- If the last 2 assistant replies began with a filler or the same pattern, start this answer "
            "differently: jump straight to the fact, use a brief topic hook, or a question — "
            "but never the same opener.\n"
            "- Use discourse fillers (well, so, hmm, umm) at most once per answer, and never as the first word "
            "if a recent assistant message in `conversation_history` also opened with a filler.\n"
            "- Rotate opening styles across turns: direct answer; short context hook; soft uncertainty phrase "
            "(\"From what the docs show...\") — pick one that differs from recent assistant openings.\n"
            "\n"
            ...
            "- Prefer starting with the core fact or a short topic hook — not a filler word.\n"
            "- Discourse fillers (well, so, basically, hmm) are optional seasoning, not a required opener. "
            "Use at most one per answer, and skip them entirely if recent assistant messages already used them.\n"
            "- Vary rhythm: not every answer needs an orienting phrase. Sometimes the best opener is the answer itself.\n"
            "- Use natural spoken transitions mid-sentence when helpful: actually, I mean, look, turns out.\n"
            ...
            "Rotate how you open and structure answers — never reuse the same template across turns:\n"
            "- Direct: lead with the core fact or answer to the question.\n"
            "- Hook: name the key entity or tension from the sources, then explain briefly.\n"
            "- Procedural: state at most 1–2 critical steps in flowing speech; defer the rest to the closing question.\n"
            "BAD: Every answer starts with the same filler or the same opening structure.\n"
            ...
            "- Compare the first 3–5 words of this answer with the openings of the last 2 assistant messages "
            "in `conversation_history`. If they match or rhyme (same filler, same structure), rewrite the opening.\n"
